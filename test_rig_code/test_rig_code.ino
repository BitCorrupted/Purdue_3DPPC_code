...
#define LEDB 4
#define LEDC 3
#define SWITCH A2 //will be used in digital mode
...
void loop() {
  ...
  int switch_pressed = digitalRead(SWITCH);
  ...
  if (switch_pressed && (is_testing == false)) {
    ...
    if (dataFile) {
      dataFile.println("Begin test " + String(test_number) + " of session. Sample delay is " + String(sample_delay));
      dataFile.close();
    }
  }
...
void update_boxcar(float sensor_value) {
  ...
  if(filtered_in < 0){filtered_in = -1 * filtered_in;}
